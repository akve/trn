...
(function ($, _, window, document, undefined) {
    ...
    ThemifyPageBuilder = {
        ...
        _RefreshHolderHeight: function(){
            $('.themify_builder_row_content').each(function(){
                $(this).find('.themify_builder_sub_row_content').each(function(){
                    var $subcolumn = $(this).children('.themify_builder_col');
                    if($subcolumn.length>1){
                        $subcolumn.find('.themify_module_holder').css('min-height',$subcolumn.first().height());
                    }
                });
                var $cols = $(this).children('.themify_builder_col');
                if($cols.length>1){
                    $cols.find('.themify_module_holder').css('min-height',$cols.first().height());
                }
            });
        },
